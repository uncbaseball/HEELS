...
lapply(c("plyr", "dplyr", "ggplot2", "lubridate", "rvest", "stringr", "tidyr",
         "mgcv", "assertthat", "default", "plotly", "png", "crosstalk", "magrittr"), require, character.only = T)
...
## Fold these into GenerateBPBreakdown and PitcherReport2018
source("Leaderboard.R")
source("Hittter Hot Cold Plus Minus.R")

####  LOAD DATA   ####
roster <- read.csv(file = "roster.csv", stringsAsFactors = F)
ps <- roster[grep(x = roster$Position, pattern = "P"),]
hs <- roster[which(!is.element(roster$Position, c("P", "LHP", "RHP"))),]
data9 <- read.csv(file = "data/uncall.csv", stringsAsFactors = F) # load("data/uncall.RData")
pre2019 <- read.csv(file = "data/pre2019 uncall.csv", stringsAsFactors = F)
...
breakeven <- function(start, succ, fail) (start - fail) / (succ - fail)
...
get.opp.from.game <- function(games) trimws(gsub(gsub(games, pattern = "[0-9]| |-|/|Game [12] of Double Header", replacement = "", ignore.case = T), pattern = "([A-Z])", replacement = " \\1"))

abbrev.pitch.types <- function(x) ifelse(x == "Fastball", "FB", ifelse(x == "ChangeUp", "CH", ifelse(x == "Curveball", "CU", ifelse(x == "Slider", "SL", x))))
...
plot.pitches <- function(data = data.frame(), show.strikezone = T, show.real.strikezone = T, shape = c("pitchtype", "Game", "ABNum", 16), type = c("PlateLoc", "Contact"),
                         star.fb = F, star.lhb = F, star.2k = F, star.xbh = F, star.hit = F, p.view = F, pitch.numbers = T, bside = "Right", pitch.numbers.col = "orange",
                         plot.title, give.axes = F, point.size = 2.4, xlims = c(-3, 3), ylims = c(-0.5, 5), pitch.type.legend = T,
                         legend.type = c("full", "take", "contact result", "full with contact result"), legend.border = "black", legend.cex = 1.2, legend.pars = list(legend.border = "black", legend.cex = 1.2, x = "bottomright")){
  ...
  #### Strike Zones ####
  plot(strikezone$x, strikezone$z, type = ifelse(show.strikezone, "l", "n"), xlim = xlims, ylim = ylims, axes = give.axes, col = "red", main = ifelse(missing(plot.title), ifelse(nrow(data) == 0, "", paste(mode(data$Pitcher), "vs.", mode(data$Batter))), plot.title), xlab = "", ylab = "", lwd = 3)#ifelse(p.view, "Pitcher View", "Catcher View")
  ...
  if(nrow(data) > 0){
    ...
    if(star.xbh & sum(data$PlayResult %in% c("Double", "Triple", "HomeRun")) > 0){
      points(data[which(data$PlayResult %in% c("Double", "Triple", "HomeRun")), "PlateLocSide"] * ifelse(p.view, 1, -1), data[which(data$PlayResult %in% c("Double", "Triple", "HomeRun")), "PlateLocHeight"], pch = 8, col = "pink")
    }
    
    if(star.hit & sum(data$PlayResult %in% c("Single", "Double", "Triple", "HomeRun")) > 0){
      points(data[which(data$PlayResult %in% c("Single", "Double", "Triple", "HomeRun")), "PlateLocSide"] * ifelse(p.view, 1, -1), data[which(data$PlayResult %in% c("Single", "Double", "Triple", "HomeRun")), "PlateLocHeight"], pch = 8, col = "green")
    }
  ...
  #### Legends
  if(shape == "pitchtype" & pitch.type.legend){
...
avg.ev.la <- function(gameids = c(), games = c()){
  data[which((data$GameID %in% gameids | data$Game %in% games) & data$PitchCall == "InPlay" & data$BatterTeam %in% c("NOR_TAR", "NOR_TAR2")),c("Date", "Batter", "Pitcher", "Inning", "ExitSpeed", "Angle")] %>% 
...
all.ev.la <- function(gamedata){
  gamedata %>% filter(BatterTeam == "NOR_TAR" & PitchCall == "InPlay") %>%
    select(Batter, Pitcher, Inning, Balls, Strikes, `Result` = PlayResult, HitType, `Launch Angle` = Angle, `Exit Velo` = ExitSpeed)
}
...
add.predicted.columns <- function(data, tagpitches = T, livebp = F, scrim = F, opponent = "", export = F){
  ...
  ## Add Columns
  if(tagpitches) data <- tag.with.df(data) # Pitch Tagging
  
  data <- givetmdataids(data, livebp = livebp, scrim = scrim) # GameID/PlayID

  data %<>% mutate(yyyymmdd = paste0("20", str_sub(as.Date(Date, format = "%m/%d/%Y"), 3, -1)),
                   key = paste(PlayID, leading0(PitchofPA)),
  ...
  data %<>% mutate(XCoord = XCoord * ifelse(BatterSide == "Left", -1, 1),
                   ...
                   Pitch_TM_Text = paste("Result:", Pitch_Result, "<br>Pitch:", TaggedPitchType, "<br>Velo:", round(RelSpeed), "<br>Count:", Count, "<br>Date:", Date, "<br>Batter:", Batter, "<br>Inning:", Inning),
                   mvmt.x = (HorzBreak / 12) * ifelse(T, 1, -1), mvmt.z = (2.525 + InducedVertBreak / 12),
                   ...
                   BBtype = ifelse(is.na(Angle) | is.na(ExitSpeed) | is.na(Distance) | is.na(Bearing), NA, 
                                   ifelse(sqrt((cos((90 - Bearing) * pi / 180) * Distance)^2 + ((sin((90 - Bearing) * pi / 180) * Distance) - 60.5)^2) <= 75.89 & Angle < 20, "GB", 
                                          ifelse((182.05 - 1.18364 * sin(Angle * pi / 180) * ExitSpeed) >= 135, "LD", 
                                                 ifelse((182.05 - 1.18364 * sin(Angle * pi / 180) * ExitSpeed) >= 108, "FB", "PU")))),
                   Year = substr(yyyymmdd, 1, 4),
                   Month = paste(month(yyyymmdd, label = T), Year),
                   Scrimmage = ifelse(startsWith(GameID, "1NTRSQ"), "Scrimmage", 
                                      ifelse(startsWith(GameID, "L1V3BP"), "Live BP", "Season")),
                   Game = paste(Date, ifelse(scrim, "Scrimmage", ifelse(livebp, "Live BP", opponent)), sep = " - ")
  ) %>%
  ...
  data %<>% left_join(., hitterhc, by = "Batter", suffix = c("", ".Hot")) %>%
    mutate(Swing = PitchCall %in% c("StrikeSwinging", "FoulBall", "InPlay"),
           ...
           In_Strike_Zone = (PitchCall == "StrikeCalled" | (!is.na(Exp.Called.Strike) & Exp.Called.Strike > 0.5)),
           HitterHotColdPM = ifelse(Strikes < 2,
                                   ...
                                   ## 2 Strikes
                                    ifelse(PitchCall == "InPlay",
                                           ifelse(as.logical(Hit), "Reached Base", 
                                                  ifelse(!is.na(Angle), 
                                                         ifelse(ContactType %in% c("Under", "Weak"), "Strikeout/Weak Contact", "Solid Contact"),
                                                         ifelse(HitType %in% c("LineDrive", "GroundBall"), "Solid Contact", "Strikeout/Weak Contact"))),
                                          ifelse(PitchCall == "BallCalled", 
                                                 ifelse(Balls == 3, "Reached Base", "Ball Take"),
                                                 ifelse(startsWith(PitchCall, "Strike"), "Strikeout/Weak Contact",
                                                        ifelse(PitchCall == "FoulBall", "Foul", ifelse(PitchCall == "HitByPitch", "Reached Base", "None")))))
                                   ),
  ...
  if(any(!startsWith(data$BatterTeam, "NOR_TAR") & !is.na(data$radius))){
    warning("The following non-UNC hitters were given a hot zone. They were coerced to NA")
    print(sort(unique(data[which(!startsWith(data$BatterTeam, "NOR_TAR") & !is.na(data$radius)),"Batter"])))
  }
  
  if(export){
    write.csv(data %>% filter(PitchCall != "Undefined") %>% select(Batter, Pitcher, Inning, Top.Bottom, Balls, Strikes, Outs, Result = PitchCall, `Pitch Type` = AutoPitchType, Velo = RelSpeed) %>% mutate(Velo = round(Velo)),
              file = paste(opponent, "at UNC", mode(data$yyyymmdd), ".csv"), row.names = F)
  }
  
  return(data)
}

pitcher.strikes.by.pitch <- function() data9 %>% filter(PitcherTeam == "NOR_TAR" & PitchCall != "Undefined" & TaggedPitchType != "Undefined") %>% group_by(Pitcher, TaggedPitchType) %>% summarise(`Strike%` = mean(as.numeric(PitchCall %in% c("StrikeCalled", "StrikeSwinging", "FoulBall", "InPlay")) - 0.5 * as.numeric(PitchCall == "FoulBall" & Strikes == 2), na.rm = T), Whiffs = sum(PitchCall == "StrikeSwinging", na.rm = T), "Total Num Thrown" = n())


synergy.pitch.chart.upload <- function(game, filename){
  write.csv(game %>% filter(PitchCall != "Undefined") %>% select(Batter, Pitcher, Inning, Top.Bottom, Balls, Strikes, Outs, Result = PitchCall, `Pitch Type` = AutoPitchType, Velo = RelSpeed),
            file = filename, row.names = F)
}


unc.vs.opp.ev <- function(){
  write.csv(data9 %>% filter(PitchCall == "InPlay") %>% group_by(Game, yyyymmdd) %>% 
              summarise(`UNC Avg Exit Velo` = round(mean(ExitSpeed * if_else(BatterTeam == "NOR_TAR", 1, NA_real_), na.rm = T)),
                        `Opponent Avg Exit Velo` = round(mean(ExitSpeed * if_else(BatterTeam != "NOR_TAR", 1, NA_real_), na.rm = T))
              ) %>% arrange(desc(yyyymmdd)) %>% select(-yyyymmdd), file = "data/UNC vs. Opp Avg Exit Velo by Game.csv", row.names = F)
}
